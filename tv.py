...
def obter_todos_videos_da_pasta():
    urls = []
    try:
        # Busca estritamente focada na pasta video_clipes do Cloudinary
        next_cursor = None
        while True:
            params_query = {"type": "upload", "resource_type": "video", "prefix": "video_clipes", "max_results": 500}
            if next_cursor:
        ...
        print("Erro ao buscar vídeos na pasta video_clipes:", e)
        
    # Fallback caso a busca com prefixo falhe por políticas de diretório
    if not urls:
        try:
...
# 3. TELA PRINCIPAL: FILA EM TEMPO REAL E VÍDEOS CLIPES
    ...
    with cl1:
        ...
        # Bloco com atualização em tempo real por JavaScript para eliminar o atraso e piscar da tela
        st.markdown("""
            <div id="lista-fila-container" style="color: white;">
                A carregar fila...
            </div>
            <script>
                function atualizarFilaRealTime() {
                    fetch('""" + URL_PEDIDOS + """?nocache=' + Date.now())
                        ...
                        .then(data => {
                            ...
                            if (!data) {
                                container.innerHTML = "<div style='color: #aaa; font-size: 1.3rem;'>A fila está vazia. Envie músicas pelo telemóvel!</div>";
                                return;
                            }
                            let html = "";
                            let contador = 1;
                            for (const [p_id, p] of Object.entries(data)) {
                                if (p.musica && !p.musica.startsWith("PEDIDO:")) {
                                    html += `<h3 style='margin: 10px 0; font-size: 1.3rem;'>${contador}. <span class='cantor-style' style='color:white; font-weight:bold;'>${p.cantor.toUpperCase()}</span> ➔ <span class='musica-style' style='color:yellow; font-weight:bold;'>${p.musica.toUpperCase()}</span></h3>`;
                                    contador++;
                                }
                            }
                            if (contador === 1) {
                                container.innerHTML = "<div style='color: #aaa; font-size: 1.3rem;'>Ainda sem cantores na fila.</div>";
                            } else {
                ...
                setInterval(atualizarFilaRealTime, 1000);
            </script>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
            ...
            <script>
                ...
                if (!window.__clipeIniciado) {{
                    ...
                    setTimeout(iniciarPlayerClipe, 300);
                }}
