    ...
    if url_video:
        st.markdown(f"""
            <div class="video-container">
                <h2 style="color: yellow; margin-bottom: 10px;">🎤 A TOCAR: {str(res_status.get('musica', '')).upper()}</h2>
                <video width="85%" controls autoplay playsinline style="border:8px solid gold; border-radius:15px; background: #111;">
        ...
        # Botão físico de fallback caso o navegador bloqueie o autoplay automático
        col_esp1, col_btn, col_esp2 = st.columns([2, 2, 2])
        with col_btn:
            if st.button("🔄 FORÇAR / RECARREGAR VÍDEO", use_container_width=True):
                st.rerun()

        # Loop silencioso para manter o ecrã estável enquanto toca
        while True:
            time.sleep(3)
            try:
        ...
        st.error("⚠️ Comando 'play' recebido, mas o link do vídeo (url_video) está vazio no Firebase.")
        time.sleep(4)
        st.rerun()
...
# 2. VEZ DO CANTOR (AGUARDANDO START)
    ...
    st.markdown(f"""
        <div style='text-align:center; padding:40px; color:white;'>
            ...
            <h3 style="color: #00ff00; margin-top: 20px;">Aguardando o cantor carregar no botão no telemóvel...</h3>
        </div>
    """, unsafe_allow_html=True)
    time.sleep(2)
    st.rerun()
...
# 3. CABEÇALHO PADRÃO E FILA DE ESPERA
    ...
    st.markdown("<h1 style='text-align:center; color:white; margin-top: 20px;'>FF KARAOKE - SALA</h1>", unsafe_allow_html=True)
